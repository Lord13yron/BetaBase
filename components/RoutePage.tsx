...
export default function RoutePage({
  ...
  const uploadUrl = `/upload?${new URLSearchParams({ routeId, gymId })}`;
  return (
    <>
      <RoutePageHeader route={route} gym={gymId} videos={videos} />
      {/* ── Videos Section ── */}
      ...
      <section id="upload" className="mx-10 mt-12">
        <div className="flex items-center justify-between flex-wrap gap-5 px-8 py-7 bg-granite rounded-xl border border-clay/20">
          ...
          <Link
            className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-clay text-white font-mono text-[12px] tracking-widest uppercase border-none cursor-pointer transition-all duration-150 hover:opacity-85 hover:-translate-y-px"
            href={uploadUrl}
          >
            <UploadIcon size={15} />
            Upload Beta
