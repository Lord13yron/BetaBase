...
//   function handleDelete(gymId: number, gymName: string) {
//     if (!confirm(`Remove "${gymName}"? This cannot be undone.`)) return;
//     startTransition(async () => {
...
//               {["Gym", "Location", "Types", "Routes", "Videos", ""].map((h) => (
//                 <th
//                   key={h}
...
//                   <button
//                     onClick={() => handleDelete(Number(gym.id), gym.name)}
//                     disabled={isPending}
            ...
            {filtered.map((gym) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-right">
                  <button
                    onClick={() =>
                      setDeleteTarget({ id: Number(gym.id), name: gym.name })
                    }
                    disabled={isPending}
                    className="px-2.5 py-1 text-[10px] tracking-wider uppercase border border-red-200 text-red-600 rounded hover:bg-red-50 transition-colors disabled:opacity-40"
                  >
